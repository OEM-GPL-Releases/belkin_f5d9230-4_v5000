		 ...
		 struct ip_nat_info *info)
{
	...
	{
		newip = newsrcip;
		DEBUGP("autofw_nat_expected: src %u.%u.%u.%u\n", NIPQUAD(newsrcip));
	}	
	else {
		...
		newip = newdstip;
		DEBUGP("autofw_nat_expected: dst %u.%u.%u.%u\n", NIPQUAD(newdstip));
	}
...
autofw_expect(struct ip_conntrack *ct)
{
	...
	DEBUGP("autofw_nat_expect: exp->tuple.src.ip %u.%u.%u.%u\n", NIPQUAD(exp->tuple.src.ip));
#endif
       ...
       void *userinfo)
{
	...
	for (port = ntohs(info->dport[0]); port <= ntohs(info->dport[1]); port++) {
		...
		DEBUGP("NEW_PORT_TRIG:autofw_expect: expecting ");
		DUMP_TUPLE(&exp->tuple);
	...
	DEBUGP("target: exp->tuple.src.ip %u.%u.%u.%u\n", NIPQUAD(exp->tuple.src.ip));
	exp->tuple.dst.protonum = info->proto;
	exp->mask.src.ip = 0xFFFFFFFF;
	exp->mask.dst.protonum = 0xFFFF;
	exp->expectfn = autofw_expect;

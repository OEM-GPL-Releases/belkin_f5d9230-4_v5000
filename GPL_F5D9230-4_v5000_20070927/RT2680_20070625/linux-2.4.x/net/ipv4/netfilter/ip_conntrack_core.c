...
//Following is modified by ChungLiang 92.9.4
     if ( tuple->dst.protonum == 50 ) { //This is for ESP
          ret = (ntohl(tuple->src.ip + tuple->dst.ip + tuple->dst.protonum))
               % ip_conntrack_htable_size;
     }
     else if ( tuple->dst.protonum == 17 && ntohs(tuple->dst.u.udp.port) == 1701
		 ) { // L2TP
          DEBUGP("L2TP hash(%u)\n", tuple->dst.u.udp.fakeTID + tuple->src.u.udp.fakeTID);
          ret = (ntohl(tuple->dst.u.udp.fakeTID + tuple->src.u.udp.fakeTID
                    + tuple->src.ip + tuple->dst.ip + tuple->dst.protonum)) % ip_conntrack_htable_size;
     }
     else if ( tuple->dst.protonum == 47 ) { //This is for GRE
          ret = (ntohl(tuple->src.ip + tuple->dst.ip
                     + tuple->src.u.gre.key 
                     + tuple->dst.u.gre.key 
     ...
     DEBUGP("(%s) proto 0x%x return 0x%x\n",__FUNCTION__,tuple->dst.protonum,ret );
     return ret;
#if 0
                ...
                struct ip_conntrack_protocol *protocol)
{
	...
	tuple->dst.protonum = iph->protocol;
        /*clear all , so we don't go into infinite loop */
        tuple->src.u.all = tuple->dst.u.all =0 ;
	/* steven: fill out layer4 port info */
...
/* Added by Steven Liu */
conntrack_tuple_cmp2(const struct ip_conntrack_tuple_hash *i,
		    const struct ip_conntrack_tuple *tuple,
		    const struct ip_conntrack *ignored_conntrack)
{
    MUST_BE_READ_LOCKED(&ip_conntrack_lock);
    return i->ctrack != ignored_conntrack
	&& ip_ct_tuple_equal2(tuple, &i->tuple);
}
/* --- */
...
/* Added by Steven Liu */
static struct ip_conntrack_tuple_hash *
__ip_conntrack_find2(const struct ip_conntrack_tuple *tuple,
	const struct ip_conntrack *ignored_conntrack)
{
    struct ip_conntrack_tuple_hash *h;
    unsigned int hash = hash_conntrack(tuple);
...
/* Find a connection corresponding to a tuple. */
struct ip_conntrack_tuple_hash *
ip_conntrack_find_get2(const struct ip_conntrack_tuple *tuple,
	const struct ip_conntrack *ignored_conntrack)
{
    struct ip_conntrack_tuple_hash *h;
    ...
    h = __ip_conntrack_find2(tuple, ignored_conntrack);
    if (h)
	atomic_inc(&h->ctrack->ct_general.use);
    READ_UNLOCK(&ip_conntrack_lock);
...
int
ip_conntrack_tuple_taken2(const struct ip_conntrack_tuple *tuple,
			 const struct ip_conntrack *ignored_conntrack)
{
	struct ip_conntrack_tuple_hash *h;
...
static inline void get_wan_port(unsigned char *name)
{
		  ...
		  enum ip_conntrack_info *ctinfo)
{
	 ...
	 *	       <--------------(III)
	 *
	 * (case I/II) cmp tuple=src_ip/port & dst_ip/port & proto
	 * (case III)  cmp tuple=dst_ip/port & proto (Full cone)
	 *                      =dst_ip/port & proto & src_ip (Restricted cone)
	 *
	 */
	get_wan_port(wan_name);
	if( skb->dev!=NULL && (strcmp(skb->dev->name, wan_name)==0)){ /* case 3*/
	    h = ip_conntrack_find_get2(&tuple, NULL);
	}else{ /* case 1.2 */
	    h = ip_conntrack_find_get(&tuple, NULL);
	}
#endif
...
// Added by ChungLiang for ESP on 92.9.4
     #ifdef CONFIG_NETFILTER_DEBUG
	DEBUGP("In resolve_normal_ct, after ip_conntrack_find_get.\n");
	DUMP_TUPLE((&h->tuple));
     #endif

     if (proto->proto == 50 && (DIRECTION(h) == IP_CT_DIR_REPLY) && h->tuple.src
.u.spi==0) {
          h->tuple.src.u.spi = tuple.src.u.spi;
     }
...
int __init ip_conntrack_init(void)
{
	...
	//This is added by ChungLiang on 92.9.4

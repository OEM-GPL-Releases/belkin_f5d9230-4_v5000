...
void prom_init_sysclk(void)
{
 /* New add by kaiker */
	...
	clk_sel = (reg>>20) & 0x03;
	switch(clk_sel) {
#ifdef CONFIG_RALINK_RT2880_VER_A
		case 0:
		...
		case 3:
			...
			break;
#elif CONFIG_RALINK_RT2880_VER_B 
		case 0:
			mips_cpu_feq = (250*1000*1000);
			break;
		case 1:
...
void prom_init_serial_port(void)
{
	...
	*/
	if (early_serial_setup((struct serial_struct *) &serial_info) != 0)
		printk("Palmchip Debug: Serial Port 0 Config failed\n"); 
	...
	*/
	if (early_serial_setup((struct serial_struct *) &serial_info) != 0)
		printk("Palmchip Debug: Serial Port 1 Config failed\n"); 
...
#error Please Choice Chip Version (A/B)
	...
	prom_printf(" - VERSION A\n");
#elif defined(CONFIG_RALINK_RT2880_VER_B)
	prom_printf(" - VERSION B\n");
#else
#error Please Choice Chip Version (A/B)
#endif

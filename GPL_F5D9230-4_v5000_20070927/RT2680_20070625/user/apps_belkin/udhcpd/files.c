...
#include "options.h"
#include "leases.h"
#include "./gemtek_added_SRC/gemtek_jimmy_added_003.h"
...
void write_leases(void)
{
	...
	Jimmy_Added_003_Extra_SRC_001
	
	if (!(fp = fopen(server_config.lease_file, "w"))) {
		LOG(LOG_ERR, "Unable to open %s for writing", server_config.lease_file);
		return;
	}
	Jimmy_Added_003_Extra_SRC_002
	Jimmy_Added_003_Extra_SRC_003
	
	for (i = 0; i < server_config.max_leases; i++) {
		
		//fprintf(stderr, "%d : %02X:%02X:%02X:%02X:%02X:%02X %X %X %s\n", i, leases[i].chaddr[0], leases[i].chaddr[1], leases[i].chaddr[2], leases[i].chaddr[3], leases[i].chaddr[4], leases[i].chaddr[5], leases[i].yiaddr, lease_time, leases[i].hostname);
		if (leases[i].yiaddr != 0) {
			Jimmy_Added_003_Extra_SRC_005
			Jimmy_Added_003_Extra_SRC_006
			if (server_config.remaining) {
				if (leases[i].expires < curr)		// if (lease_expired(&(leases[i])))
					lease_time = 0;

...
#define _PATH_ETHOPT         _ROOT_PATH "/var/ppp/options."
//      --- Gamble, GTK_PPPoE
...
// it will make pppd get an segmentation fault signal and
// pppd will be killed by itself
//	warn("Couldn't increase MTU to %d", mtu);
        notice("Couldn't increase MTU to %d\n", mtu);
// --- Gamble, GTK_PPPoE
	mtu = MAX_PPPOE_MTU;
    }
...
//	warn("Couldn't increase MRU to %d", mru);
        notice("Couldn't increase MRT to %d\n", mru);
// --- Gamble, GTK_PPPoE
      }
}
...
PPPoEDevnameHook(char *cmd, char **argv, int doit)
{
    ...
    else if (strlen(cmd) < 4 || strncmp(cmd, "vlan", 4) && (strncmp(cmd, "eth", 3) && strncmp(cmd, "nas", 3) && strncmp(cmd, "tap", 3) && strncmp(cmd, "ppp", 3))) 
    {
//	+++ Gamble, GTK_PPPoE
// This is special case for Atheros AP51 Linux based Project,
// If the name of your wan interfeace is starting with 'eth',
// you should not have to add the following code
		if (strncmp(cmd, "ae", 2))
		{
// --- Gamble, GTK_PPPoE		    	
			if (OldDevnameHook) 

...
int pmlist_AddPortMapping (int enabled, char *protocol, char *externalPort, char *internalClient, char *internalPort)
{
	...
	
	{
	  ...
	  //mavis:check if port is digit or not
	  trace(3, "pmlist_AddPortMapping %s", externalPort);
	  trace(3, "pmlist_AddPortMapping:Test digit");
	  for( i = 0 ; externalPort[i] != 0 ; i++ )
	  {
	  	if( !isdigit(externalPort[i]) )
	  		return 1;
	  }	
	  for( i = 0 ; externalPort[i] != 0 ; i++ )
	  {
	  	if( !isdigit(internalPort[i]) )
	  		return 1;
	  }	
	  
  	if((atoi(externalPort) <65536)&&(atoi(externalPort)>0)&&(atoi(internalPort)<65536)&&(atoi(internalPort)>0))
  	{
	...
	if (g_vars.forwardRules)
	{
	  ...
	  if( (atoi(internalPort)<65536) && (atoi(internalPort)>0) )
	  {	
...
int pmlist_DeletePortMapping(int enabled, char *protocol, char *externalPort, char *internalClient, char *internalPort)
{
  ...
  //mavis:check if port is digit or not
	...
	if (g_vars.forwardRules)
	{
	 ...
	 if((isdigit(internalPort))&&(atoi(internalPort)<65536)&&(atoi(internalPort)>0))
	  {	 

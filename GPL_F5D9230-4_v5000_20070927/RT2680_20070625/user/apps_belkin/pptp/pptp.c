 ...
 *
 * $Id: pptp.c,v 1.1.1.1 2004/04/28 07:18:26 tom Exp $
 */
...
#ifndef PPPD_BINARY
#define PPPD_BINARY "/bin/pppd"	// Andy Modi
#endif
#define ANDYDEBUG
...
int setup_usr_pwd(void)
{
	FILE	*sd;
	char 	usr[10];
	int		len;
#define PPP_OPTION_FILE "/var/ppp/options"

	strcpy(usr, "testusr");
	len = strlen(usr);
	printf("len is %d \n", len);
	usr[len] = '"';
	sd = fopen(PPP_OPTION_FILE, "w");
	if (sd ==0 ){
...
{
#if 0
	int redialtime, demand, keepalive, pptp_status, pptp_enable;
	...
	//i=setup_usr_pwd();
	//if (i<0)
	//	printf("wrong access usrname and password\n");

	redialtime 	= atoi(nvram_safe_get("pptp_redialtime"));
	demand		= atoi(nvram_safe_get("pptp_demand"));
	keepalive 	= atoi(nvram_safe_get("pptp_keepalive"));
	pptp_status	= atoi(nvram_safe_get("pptp_status"));
	pptp_enable	= atoi(nvram_safe_get("pptp_enable"));
	...
	if (demand == 1)
	{
		nvram_set("pptp_status", "2");
		pptp_main(argc, argv, envp);
		nvram_set("pptp_status", "0");
	}

	if (keepalive ==1)
	{
		//for(i=0;i<redialtime;i++){
		for(;;){
			nvram_set("pptp_status", "2");
			pptp_main(argc, argv, envp);
			nvram_set("pptp_status", "0");
			usleep(redialtime*1000000);
		}
	}
#endif
pptp_main(argc, argv, envp);
...
#ifdef ANDYDEBUG
  printf("Step 1a: Get IP address for the hostname in argv[1] \n");
#endif
  inetaddr = get_ip_address(argv[1]);

  /* step 1b: Find the ppp options, extract phone number */
#ifdef ANDYDEBUG
  printf("step 1b: Find the ppp options, extract phone number \n");
#endif
  argc--;
  argv++;
  while(1){
...
#ifdef ANDYDEBUG
	printf(" Step 3: Find an open pty/tty pair.\n");
#endif
  if(launchpppd){
      rc = open_pty (&pty_fd, &tty_fd, ttydev, NULL, NULL);
#ifdef ANDYDEBUG
	printf("The PTY fd is %d and TTY fd is %d \n",pty_fd, tty_fd);
#endif
      if (rc < 0) {
          close(callmgr_sock);
	...
	printf("fork and wait \n");
      ...
      case -1:
        //fatal("Could not fork pppd process");
			printf("Could not fork pppd process");
      case 0: /* I'm the child! */
        close (tty_fd);
#ifdef ANDYDEBUG
	printf("tty_fd is closed \n");
#endif
  ...
  do {
	...
	if (callmgr_sock < 0){
		...
		kill(parent_pid, SIGTERM);
	  	close(pty_fd);
  		close(callmgr_sock);
		printf("Unable Connect PPTP Gateway, Check IP setting \n");
		close(callmgr_sock);
		//exit(0) ;
		return -1;
	}
...
#ifdef ANDYDEBUG
	printf("Step 5: Exchange PIDs, get call ID \n");
#endif
...
#ifdef ANDYDEBUG
	printf("Step 5b: Send signal to wake up pppd task \n");
#endif
  if(launchpppd){
...
#ifdef ANDYDEBUG
	printf("Step 6: Do GRE copy until close and pty_fd is %d.\n", pty_fd);
#endif
  //nvram_set("pptp_status", "1");
  pptp_gre_copy(call_id, peer_call_id, pty_fd, inetaddr);

shutdown:
  /* on close, kill all. */
#ifdef ANDYDEBUG
	printf("on close, kill all.\n");
#endif
  if(launchpppd)
      kill(parent_pid, SIGTERM);
  nvram_set("pptp_status", "0");		//Andy Add 20030227
  printf("Closing pty file descrip\n");
  close(pty_fd);
  close(callmgr_sock);
...
int open_callmgr(struct in_addr inetaddr, char *phonenr, int argc, char **argv, char **envp)
{
  ...
  if ((fd = socket(AF_UNIX, SOCK_STREAM, 0)) < 0) {
    printf("Could not create unix domain socket: %s", strerror(errno));
  }
  ...
  for (i=0; i<NUM_TRIES; i++) {
    if (connect(fd, (struct sockaddr *) &where, sizeof(where)) < 0) {
      ...
      switch (pid=fork()) {
      case -1: /* failure */
	  printf("fork() to launch call manager failed.");
      case 0: /* child */
	  {
	      close (fd);
	  ...
	  if (status!=0){
			close (fd);
	  	    printf("Call manager exited with error %d\n", status);
			return -1;
		}
	  break;
      }
  ...
  close(fd);
  printf("Could not launch call manager after %d tries.", i);
...
void launch_pppd(char *ttydev, int argc, char **argv) {
  ...
  printf("PPPD_BINARY = %s \n",PPPD_BINARY);

//exit(0); // andy debug 20021203
#ifdef USER_PPP
  new_argv[i++] = "-direct";
  /* ppp expects to have stdin connected to ttydev */
  if ((j = open(ttydev, O_RDWR)) == -1)
    printf("Cannot open %s: %s", ttydev, strerror(errno));
  if (dup2(j, 0) == -1)

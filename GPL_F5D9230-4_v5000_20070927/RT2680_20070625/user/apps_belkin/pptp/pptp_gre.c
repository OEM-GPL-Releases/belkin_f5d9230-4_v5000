 ...
 *
 * $Id: pptp_gre.c,v 1.1.1.1 2004/04/28 07:18:26 tom Exp $
 */
...
#define PACKET_MAX 8196  // PC version
//#define PACKET_MAX 128		// embeded cpu version
...
/*
#ifdef ANDYDEBUG
	printf("the retval of select is %d and errno is %d\n", retval,errno);
#endif
*/
 ...
 //       break;
	if (FD_ISSET(pty_fd, &rfds)&& (decaps_hdlc(pty_fd, encaps_gre, s) < 0))
	{
		printf("retval of decaps_hdlc is %d and errno is %d %s \n", retval, errno , strerror(errno));
		break;
	}

	if (FD_ISSET(s,  &rfds) && (decaps_gre(s, encaps_hdlc, pty_fd) < 0))
	{
		printf("retval of decaps_s is %d and errno is %d \n", retval, errno);
		break;
	}
...
#ifdef ANDYDEBUG
	printf("short read (%d): %s and errno is %d\n", end, strerror(errno), errno);
#endif	
		if (errno ==2){
		strcpy(ptydev, PTYDEV);
			...
			{
				printf("ptydev %s , file descrip is %d \n",ptydev, fd1);
				goto ckagain;
			}
			printf("Fail to reopen pty device\n");
		}

    return -1;
  }
#ifdef ANDYDEBUG
  printf("number of byte reading from port is %d \n", end);
#endif
  while (start < end) {

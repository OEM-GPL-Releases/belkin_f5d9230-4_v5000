 ...
 *
 * $Id: pptp_callmgr.c,v 1.1.1.1 2004/04/28 07:18:26 tom Exp $
 */
#include <signal.h>
#include <sys/time.h>
#include <sys/types.h>
...
int main(int argc, char **argv, char **envp) {
  ...
  if (argc < 2) 
    printf("Usage: %s ip.add.ress.here [--phone <phone number>]", argv[0]);
  ...
  if ((inet_sock = open_inetsock(inetaddr)) < 0){
    printf("Could not open control connection to %s", argv[1]);
	return -1;
  }
  if ((unix_sock = open_unixsock(inetaddr)) < 0){
    printf("Could not open unix socket for %s", argv[1]);
	return -1;
  }
  ...
  { 
    ...
    if (conninfo==NULL) {
      close(unix_sock); close(inet_sock); printf("No memory.");
    }
  ...
  do {
	...
	printf("Socket not accepted: %s", strerror(errno));
	goto skip_accept;
      }
      /* Allocate memory for local call information structure. */
      if ((lci = malloc(sizeof(*lci))) == NULL) {
	printf("Out of memory."); close(s); goto skip_accept;
      }
      lci->unix_sock = s;
...
int open_unixsock(struct in_addr inetaddr) {
  ...
  if (stat(where.sun_path, &st) >= 0) {
    printf("Call manager for %s is already running.", inet_ntoa(inetaddr));
    close(s); return -1;
  }
  ...
  if (!make_valid_path(dir, 0770))
    printf("Could not make path to %s: %s", where.sun_path, strerror(errno));
  free(dir);

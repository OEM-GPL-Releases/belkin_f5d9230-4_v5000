...
#else
#define sdebug( fmt , args... )	fprintf(stderr,fmt"\n", ##args )
#endif
...
int main(int argc, char **argv)
{
	...
	router_ip = htonl( inet_network( nvram_safe_get("lan_ipaddr") ) ) ;
	cwf_timeout_timer = atoi( nvram_safe_get( "cwf_timeout_timer" ) );
	cwf_timeout_action = atoi( nvram_safe_get( "cwf_timeout_action" ) );
...
#endif	
	if( strlen(nvram_safe_get("cwf_serial_number")) > 0  )
		strcpy( BEL_SerialNumber, nvram_safe_get("cwf_serial_number") );
	else	
		strcpy( BEL_SerialNumber, "BEL151111" );
	/*
...
{	
	...
	for (;;)
	{
		...
		if( strlen(nvram_safe_get("cwf_parental_mac")) > 0 && !forked )
		{
			forked = 1;
			switch (pid = fork())
			{
		...
		if( strlen(nvram_safe_get("cwf_parental_mac")) == 0)
		{
			forked = 0;
		}	
						...
						char *server_hostname, short server_port)
{
	...
	if( (conn.client_addr.sin_addr.s_addr & 0x00FFFFFF) != (router_ip & 0x00FFFFFF) )
		exit(0);
...
int confirm_client_request_with_cwf(connection_t *conn, size_t last_read )
{
   ...
   FILE  *fp;
	...
	unblock_mac = 0 ; 
	if( strlen(nvram_safe_get("cwf_parental_mac")) > 0 && !strcmp( conn->client_mac_str ,nvram_safe_get("cwf_parental_mac")) )
		    unblock_mac =1;
	...
	if( strstr( cwfbuf , "GET") && cwft->cwf_timeouted==0 && !unblock_mac )
	{
		...
		if(  cwf_host_count > 1 )
		{
			...
			{
				if( (conn->cwf_fd = connect_to_cwf( ntohl( inet_network(cwf_ip[i]) ) , ntohs(cwf_port) )) != -1 )
					break;
			}	
		}
		else

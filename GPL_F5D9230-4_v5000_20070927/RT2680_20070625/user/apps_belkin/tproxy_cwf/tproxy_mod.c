...
#ifndef __EMBEDDED__
/* Requests must fit in this size */
#define BUFFER_SIZE			(32769)
#else
/* Request must fit in this size */
...
#define sdebug(x)
#else
#define sdebug(x)	fprintf(stderr,"%s\n",x);
#endif
...
static int connect_to_proxy(connection_t *conn,ip_address_t ip, short port);
...
static int connect_to_proxy(connection_t *conn,ip_address_t ip, short port)
{
	...
	if (connect(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0)
	{
		...
		get_ip_address("br0",&myip);
		
		sprintf(buf2,"<HTML><HEAD><META HTTP-EQUIV=REFRESH CONTENT=\"0;URL=http://%s\"></HEAD><TITLE></TITLE><BODY>REDIRECT TO ROUTER WEB</BODY></HTML>",inet_ntoa(myip));
		sprintf(buf,"HTTP/1.1 200 OK\r\nServer: thttpd0.9.8\r\nContent-type: text/html\r\nDate:\r\nLast-modified:\r\nAccept-Ranges: bytes\r\nConnection: keep-alive\r\n\r\n%s",buf2);
		write(conn->client_fd , buf , strlen(buf));
		
		read(conn->client_fd,buf,sizeof(buf)) ;
...
sdebug("Before -- Accept an incoming connection.")
		while ((new_sock = accept(sock, (struct sockaddr *)&addr, &len)) < 0)
		{
			 ...
			 */
sdebug("Connection resets are common enough to log them as debug only.")
...
#else /* EMBEDDED */ /* For running simple process now */
sdebug("Running as a single process now")
			trans_proxy(new_sock, &addr, server_hostname, server_port);
			//close(new_sock);
sdebug("Connection completed")
#endif
			}
		}
...
sdebug("Perform the transparent proxy activity.")
	/*
	 * Initialise the connection structure.
	 */
sdebug("Initialising the connection structure.")
	memset(&conn, 0, sizeof(connection_t));
//sdebug("memset(&conn, 0, sizeof(connection_t)")
	conn.client_fd = sock;
//sdebug("conn.client_fd = sock;")
	conn.client_addr = *from_addr;
//sdebug("conn.client_addr = *from_addr;")
	conn.read_buffer.free_space = sizeof(conn.read_buffer.buffer);
//sdebug("conn.read_buffer.free_space = sizeof(conn.read_buffer.buffer);")
	conn.request.free_space = sizeof(conn.request.buffer);
//sdebug("conn.request.free_space = sizeof(conn.request.buffer);")
	conn.parse_state = PS_METHOD;
//sdebug("conn.parse_state = PS_METHOD;")
	conn.parse_state = PS_METHOD;
//sdebug("conn.parse_state = PS_METHOD;")

sdebug("Initialised the connection structure.")
...
sdebug("NO IPTABLES")
	 ...
	 */
sdebug("!!! Trying to get the IP address the client tries to connect to")
sdebug("!!! WOW !!!  getsockname can get the dest addr... Cool !!!!")
	length = sizeof(conn.dest_addr);
	if (getsockname(sock, (struct sockaddr *)&conn.dest_addr, &length) < 0)
	{
sdebug("Error: getsockname...")
#if defined(LOG_TO_SYSLOG) || defined(LOG_FAULTS_TO_SYSLOG)
		syslog(LOG_ERR, "getsockname(): %m");
#endif
		return;
	}
sdebug("PASS: getsockname...")
#ifdef IPFILTER
sdebug("IPFILTER")
	/*
...
# endif
	 ...
	 */
sdebug("!!!!!!!!!!!!!  IMPORTANT  !!!!!!!!!!!!!!!!!")
sdebug("Looking up proxy server IP address.")
sdebug("Server_Hostname:  ")
sdebug(server_hostname)

	server_ip = getipaddress(server_hostname);
sdebug("getipaddress called")

	if (server_ip == INADDR_NONE)
	{
sdebug("server_ip == INADDR_NONE.....  ")
sdebug("Could not resolve IP address for proxy.......... return/exiting....")
#if defined(LOG_TO_SYSLOG) || defined(LOG_FAULTS_TO_SYSLOG)
	 ...
	 */
sdebug("Before --  Connect to the proxy server or \n to the original destination in the case of fully transparent mode,\n or a failed connect to the proxy")
	if (fully_transparent 
		/*|| (conn.proxy_fd = connect_to_proxy(&conn,server_ip, server_port)) == -1*/ )
	{
		if (!proxy_only)
		{
sdebug("Not a proxy only mode")
sdebug("Attempting to connect to the intended destination address")
			conn.proxy_fd = connect_to_proxy(&conn ,conn.dest_addr.sin_addr.s_addr, conn.dest_addr.sin_port);
			fully_transparent = 1;
		}
		else
		{
...
sdebug("Maybe connected???")
	if (conn.proxy_fd == -1)
	{
		//char buf[256];
sdebug("enter conn.proxy_fd == -1");		
#if defined(LOG_TO_SYSLOG) || defined(LOG_FAULTS_TO_SYSLOG)
		if (fully_transparent)
...
#endif
#if 0
		sprintf(buf,"HTTP/1.1 302 Move Temporarily\r\nLocation: http://192.168.0.10\r\nContent-type: text/html\r\nDate:\r\nLast-modified:\r\nAccept-Ranges: bytes\r\nConnection: close\r\n\r\n");
		write(conn.client_fd , buf , strlen(buf));
		close(conn.client_fd);
#endif		
		return ;

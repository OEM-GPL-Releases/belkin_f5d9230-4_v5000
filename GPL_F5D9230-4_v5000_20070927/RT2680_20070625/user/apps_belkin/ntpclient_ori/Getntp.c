...
*/
  	...
  	*cp = 0x0; cp++; 
  	if( (cp2 = strchr( cp , ':')) != NULL )
  	{
  			*cp2 = 0x0; 
  			cp2++;
  			printf("cp = %f\n" , atoi( cp ));
  		  timezone_v = atoi( cp )*-1.0 + atoi( cp2 )/60.0*-1.0;
  	}
  	else
  		  timezone_v = atoi( cp )*-1.0;	
  }	
  else if ( (cp=strchr( strrcv , '-')) != NULL )		
  {
    *cp = 0x0; cp++; 
  	...
  	{
  			*cp2 = 0x0; cp2++;
  		  timezone_v = (atoi( cp ) + atoi( cp2 )/60.0);
  	}
  	else
  		  timezone_v = atoi( cp );	
  }  
...
#ifdef TIMEZONE 		   // andy add 20021127 start
fprintf( stderr , "value of timezone is %f\n" , timezone );
		tv_set.tv_sec  = xmttime.coarse + (int)(3600*timezone_v) - JAN_1970;
...
int nationselect(void)
{
	...
	if(nation == 13)
			return 1;	
	// Atlanta - first Sunday of April to the last Sunday of October
	if(nation == 14)
			return 1;	
	//Caracas, La Paz, Bolivia - No Daylight saving
	if(nation == 15)
			return 0;	
	// Santiago - Second Sunday of October to the Second Sunday of March
	if(nation == 16)
			return 4;	
	if(nation == 17)
			return 0;	
	...
	if(nation == 24)
			return 2;
	// Amsterdam, Berlin, Bern, Rome - last Sunday of March to the last Sunday of October
	if(nation == 25)
			return 2;
	if(nation == 26)
			return 2;
	...
	if(nation == 29)
			return 2;
	// Sofija, Warsaw, Zagreb - last Sunday of March to the last Sunday of October
	if(nation == 30)
			return 2;
	if(nation == 31)
			return 0;
	...
	if(nation == 36)
			return 3;
	// Baghdad - first Sunday of April to the last Sunday of October
	// Kuwait, Nairobi, Riyadh - No Daylight saving
	if(nation == 37)
			return 1;
	// Moscow, St. Petersburg - last Sunday of March to the last Sunday of October
	if(nation == 38)
			return 2;
	// Tehran - No Daylight saving
	if(nation == 39)
			return 0;
	// Abu Dhabi, Muscat, Tbilisi - No Daylight saving
	// Kazan - last Sunday of March to the last Sunday of October
	if(nation == 40)
			return 2;
	// Volgograd, Kabul - No Daylight saving		
	if(nation == 41)
			return 0;
	// Islamabad, Karachi, Ekaterinburg - No Daylight saving	
	if(nation == 42)
			return 0;
	// Almaty, Dhaka - No Daylight saving	
	if(nation == 43)
			return 0;
	// Bangkok, Jakarta, Hanoi - No Daylight saving
	if(nation == 44)
			return 0;
	//Beijing, Chongqing, Urumqi - No Daylight saving
	if(nation == 45)
			return 0;
	// Hong Kong, Singapore, Taipei - No Daylight saving
	// Perth - last Sunday of October to the last Sunday of March
	if(nation == 46)
			return 0;
	// Toyko, Osaka, Sapporo, Yakutsk - No Daylight saving
	if(nation == 47)
			return 0;
	// Brisbane - No Daylight saving
	if(nation == 48)
			return 0;
	// Canberra, Melbourne, Sydney - last Sunday of October to the last Sunday of March
	if(nation == 49)
			return 3;
	// Guam, Port Moresby - No Daylight saving
	// Vladivostok - last Sunday of March to the last Sunday of October
	if(nation == 50)
			return 0;
	// Hobart - first Sunday of October to the last Sunday of March
	if(nation == 51)
			return 7;
	...
	if(nation == 53)
			return 0;
	// Wellington, Auckland - first Sunday of October to the third Sunday of March
	if(nation == 54)
			return 6;
	return 0;																
...
int autodaylightsave(struct timeval *tv_now)
{
	...
	case 1:								/*tm_mon range is 0~11 */
		if ( (rv_tm->tm_mon == 2))     /* Present Month is April */
		{

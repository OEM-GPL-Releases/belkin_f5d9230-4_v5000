...
#include "dnsmasq.h"
#ifdef MY_ROUTER
#define ROUTER_NAME "routerlogin.com"
char router_names[][32]={ \
  "routerlogin.com", \
  "routerlogin.net", \
  "www.routerlogin.com", \
  "www.routerlogin.net", \
...
//int is_myrouter(HEADER *header, char *limit, unsigned int qlen,char *name );
...
#ifdef MULTIPPPOE
void add_static_route(unsigned char *ansp, int count, HEADER *header, size_t plen)
{
  int i, rdlen;
  unsigned long ip_addr, ttl;
  int qtype, qclass;
  struct in_addr addr;
  ...
  for (i = 0; i < count; i++)
    {
      if (!(ansp = skip_name(ansp, header, plen)))
				return NULL; 
			syslog(LOG_INFO, "find_addr : count is %d, ansp is %x\n", count, ansp);
	    GETSHORT(qtype, ansp); 
	    GETSHORT(qclass, ansp);
	    GETLONG(ttl, ansp);
	    GETSHORT(rdlen, ansp);
	    syslog(LOG_INFO, "find_addr : qtype is %d\n", qtype);
	    if(qtype != 1)
	    	ansp += rdlen;
	    else {
				GETLONG(ip_addr, ansp);
				addr.s_addr = htonl(ip_addr);
				sprintf(request_addr, "%s", inet_ntoa(addr));
				syslog(LOG_INFO, "find_addr : query_name is %s, dns_server is %s, ip_addr is %s\n", query_name, dns_server, request_addr);

				if(!strcmp(nvram_safe_get("wan_proto"), "multipppoe")) {
					char str[60],dns_name[20],gateway_name[20];
					if(strcmp(nvram_safe_get("ppp1_wan_ipaddr"), "0.0.0.0")) {
						if(atoi(nvram_safe_get("pppoe_session2_type"))!=2) {
							if(strstr(query_name, "flets") && strcmp(query_name, "www.flets.com")) {
								sprintf(str, "route add %s gw %s", request_addr, nvram_safe_get("ppp1_wan_gateway"));
								syslog(LOG_INFO, "str %s", str);
								system(str);						
							}
						}
						else {
							char ppp_policy_type_str[30];
							char ppp_policy_rule1_str[30];
							char ppp_policy_rule2_str[30];
							int i=0;
							while(i<20) {
								sprintf(ppp_policy_type_str, "ppp_policy_type%d", i);
								sprintf(ppp_policy_rule1_str, "ppp_policy_rule%d_1", i);
								sprintf(ppp_policy_rule2_str, "ppp_policy_rule%d_2", i);
								if(atoi(nvram_safe_get(ppp_policy_type_str))==0) {
									if(strstr(query_name, nvram_safe_get(ppp_policy_rule1_str)) && strcmp(nvram_safe_get(ppp_policy_rule1_str), "")) {
										sprintf(str, "route add %s gw %s", request_addr, nvram_safe_get("ppp1_wan_gateway"));
										syslog(LOG_INFO, "str %s", str);
										system(str);
									}
					...
					/*if(strstr(nvram_safe_get("ppp1_wan_dns"), dns_server) != 0) {
						sprintf(str, "route add %s gw %s", request_addr, nvram_safe_get("ppp1_wan_gateway"));
						syslog(LOG_INFO, "str %s", str);
						system(str);
					}*/
...
#ifdef MULTIPPPOE
  add_static_route(ansp, ntohs(header->ancount) + ntohs(header->nscount) + ntohs(header->arcount), header, plen);
#endif
  if (!ansp)
    return 0;
...
int is_myrouter(HEADER *header, char *limit, unsigned int qlen,char *name )
{
  ...
  int i;
	...
	return 0; /* bad packet */
      ...
      if (( match_name && nvram_safe_get("lan_ipaddr") != NULL )  || (atoi(nvram_safe_get("dns_hijack"))==1) )
//      if ( match_name && nvram_safe_get("lan_ipaddr") != NULL  )
	{ 	
//	  unsigned long ttl = 0 ;
	  unsigned long ttl = 5 ;
	  ...
	  ans = 1;
	      ...
	      inet_aton( ipaddr , &lan_ip );
	      memcpy(ansp, &lan_ip , INADDRSZ);
	      ansp += INADDRSZ;
	      anscount++;

...
#endif 
#ifdef MULTIPPPOE
			strcpy(query_name, daemon->namebuff);
#endif
		  forwarded = 1;
			 ...
			 unsigned int query_crc, struct server *server, unsigned int n)
{
  ...
  if (header->opcode != QUERY || (header->rcode != NOERROR && header->rcode != NXDOMAIN))
    return n;
#ifdef MULTIPPPOE
	strcpy(dns_server, inet_ntoa(server->addr.in.sin_addr));
#endif
...
#ifdef MULTIPPPOE
{
	if(atoi(nvram_safe_get("pppoe_session2_on"))==0 && !strcmp(nvram_safe_get("ppp1_wan_ipaddr"), "0.0.0.0")) {
			if(strstr(daemon->namebuff, "flets") && strcmp(daemon->namebuff, "www.flets.com")) {
				//dbg_printf("Marmot : strstr(daemon->namebuff, nvram_safe_get(ppp_policy_rule1_str))\n");
				nvram_set("pppoe_session2_on", "1");
				system("ping -c 1 99.99.99.99");
			}
	}
}
#endif
